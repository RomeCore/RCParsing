...
{
	/// <summary>
	/// A class that can be built into an error recovery strategy that skips input until an anchor rule is found, with an optional stop rule.
	/// </summary>
	public class BuildableSkipUntilAnchorErrorRecoveryStrategy : BuildableErrorRecoveryStrategy
	{
		/// <summary>
		/// Gets or sets the anchor rule that will be used to determine where to stop skipping.
		/// </summary>
		...
		{
			if (ruleChildren[1] == -1)
				return new SkipUntilAnchorErrorRecoveryStrategy(ruleChildren[0], RepeatSkip);
			return new SkipUntilAnchorWithStopErrorRecoveryStrategy(ruleChildren[0], ruleChildren[1], RepeatSkip);
		}
	}
}